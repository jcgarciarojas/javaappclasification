...
public class FuzzySetTrapezoide extends FuzzySet {
	 ...
	 */
	public FuzzySetTrapezoide() throws ConfigurationException
	{
		super();
	}
	 ...
	 * @throws ConfigurationException
	 */
	public FuzzySetTrapezoide(SharpValue max) throws ConfigurationException
	{
		super(max);
	}
	...
	public void setStringValues(String strValues) throws ConfigurationException 
	{
		...
		if (l.size() != 4)
			throw new ConfigurationException("trapezoide-set must have only four values "+l);
		...
		this.getValues().add(new FuzzyValue(this.validateValue(l.get(2)), SharpValue.valueOf("1")));
		this.getValues().add(new FuzzyValue(this.validateValue(l.get(3)), SharpValue.valueOf("0")));
	}
	
	public void setArrayValues(List<FuzzyValue> values) throws ConfigurationException 
	{
		if (values.size() != 4)
			throw new ConfigurationException("size of values for this fuzzys set must be four ");
		...
		if(values.get(2).getDegree().compareTo(SharpValue.valueOf("0")) == 0)
			throw new ConfigurationException("degree of last element in this fuzzy set must be different to zero ");

		if(values.get(3).getDegree().compareTo(SharpValue.valueOf("0")) != 0)
			throw new ConfigurationException("degree of last element in this fuzzy set must be zero ");
	...
	public List<FuzzyValue> getSquareValues()
	{
		...
		l.add(this.getValues().get(2));//c
		l.add(this.getValues().get(3));//d
		return l;
	}
	...
	public SharpValue getMatchingDegree(SharpValue input) throws ConfigurationException
	{
		...
		double x4 = this.getValues().get(3).getSharpValue().getValue();
		double y4 = this.getValues().get(3).getDegree().getValue();

		if (x <= x1) y = 0D;
		else if (x >= x2 && x <= x3) y = 1D;
		else if (x >= x4) y = 0D;
		else if(x > x1 && x < x2)
			y = this.getIntersecionYPoint(x, x1, y1, x2, y2);
		else if(x > x3 && x < x4)
			y = this.getIntersecionYPoint(x, x3, y3, x4, y4);
		else 
