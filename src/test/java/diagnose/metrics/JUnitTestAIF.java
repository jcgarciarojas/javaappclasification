...
import edu.umd.msswe.ece591.diagnose.metrics.AIF;
...
public class JUnitTestAIF extends TestCase {
	...
	public void testExecuteAbstractClass() throws MetricsException, ConfigurationException
	{
		...
		Metric metric = new AIF();
		MetricReport report = metric.execute(javaApplication, javaApplication.getClassObject("ChildClass3"));
		assertTrue("validate number of methods"+report.getValue(), report.getValue() >= 0.5d);
		//assertTrue("expected 1 method", report.getDetail().size() == 5);
		report = metric.execute(javaApplication);
		assertTrue("validate number of attributes "+report.getValue(), report.getValue() >= 0.5d);
	}
	...
	{
		ResourceBundle bundle = ResourceBundle.getBundle("AifTest");
